...
define('local_edwiserreports/reports/activeusers', [
    ...
    function createModalOfUsersList(CONTEXTID) {
        $(document).on('click', SELECTOR.MODALTRIGGER, function() {
            ...
            var titleDate = rtl ? common.formatDate(new Date(eval(modalFilter * 86400 * 1000)), "yyyy MMM d", true) : common.formatDate(new Date(eval(modalFilter * 86400 * 1000)), "d MMM yyyy", true);
            ...
            ModalFactory.create({
                body: Fragment.loadFragment(
                    ...
                    CONTEXTID, {
                        page: 'activeusers',
                        filter: modalFilter,
                        cohortid: $(SELECTOR.COHORT).val() || 0,
                        action: action
                    }
                )
            }).then(function(modal) {
    ...
    function createActiveUsersTable() {
        ...
        $.ajax({
            ...
            $.each(response.dates, function(idx, val) {
                // Use UTC (true as 3rd param) for consistent date display across timezones.
                ActiveUsers[idx] = {
                    date: rtl ? '<div style="direction:ltr;">' + common.formatDate(new Date(eval(val * 86400 * 1000)), "yyyy MMM d", true) + '</div>' : common.formatDate(new Date(eval(val * 86400 * 1000)), "d MMM yyyy", true),
