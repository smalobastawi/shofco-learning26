...
define('local_edwiserreports/blocks/activeusers', [
    ...
    const lineChartDefault = {
        ...
        tooltip: {
            ...
            custom: function({ series, seriesIndex, dataPointIndex, w }) {
                // Use UTC (true as 3rd param) for consistent date display across timezones.
                let tooltip = `
                <div class="apexcharts-tooltip-title" style="font-family: Helvetica, Arial, sans-serif; font-size: 12px;">
                    ${common.formatDate(new Date(w.config.xaxis.categories[dataPointIndex]), "d MMM yyyy", true)}
                </div>`;
            ...
            .done(function(html, js) {
                ModalFactory.create({
                    ...
                    title: `${M.util.get_string('activeusersheader', 'local_edwiserreports')} (${
                        common.formatDate(new Date(context.date * 86400000), "d MMM yyyy", true)
                    })`
                }).then(function(modal) {
    ...
    // Dropdown (ellipsis menu) accessibility and event handling for block actions
    function initBlockSettingDropdowns(root) {
        if (!root) root = document;
        var dropdowns = root.querySelectorAll('.block-header .dropdown.blocksetting');
    ...
    // Call this in the block's main init
    initBlockSettingDropdowns(document);

    // Tooltip (info icon) initialization for block descriptions
    function initBlockTooltips(root) {
        if (!root) root = document;
        var tooltips = root.querySelectorAll('[data-region="tooltip"]');
        tooltips.forEach(function(el) {
            Tooltip.init(el);
        });
    }
    // Call this in the block's main init
    initBlockTooltips(document);
