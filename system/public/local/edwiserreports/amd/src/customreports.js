...
define([
    ...
    /**
     * Set column widths dynamically for custom report preview table
     * Divides 1200px total width among all columns
     */
    function setCustomReportColumnWidths() {
        var $table = $('#cr-preview-table');
        var $headers = $table.find('thead th');
        var columnCount = $headers.length;
        
        if (columnCount > 0) {
            // Calculate width per column: 1200px divided by number of columns
            var totalWidth = 1200;
            var widthPerColumn = totalWidth / columnCount;
            
            // Set width on each header column
            $headers.each(function() {
                $(this).css('width', widthPerColumn + 'px');
            });
            
            // Also set table width to 1200px
                ...
                if (response.success) {
                    ...
                    if (data.reportsdata.length == 0) {
                        ...
                        cfPreviewTable = $('#cr-preview-table').DataTable({
                            ...
                            autoWidth: false, // Ensures columns do not shrink unexpectedly
                            responsive: true, // Enables responsive column handling
                            // Maintainer: Ensure data.columns matches keys in data.reportsdata for correct column rendering
                            language: {
                            ...
                            initComplete: function() {
                                ...
                                setCustomReportColumnWidths();
                            },
                            drawCallback: function() {
                                common.stylePaginationButton(this);
                                // Re-apply column widths on each draw
                                setCustomReportColumnWidths();
                            }
                        });
    ...
    function validateCustomReprtsSaveData(data) {
        var ret = true;
        var fullnameformate = /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>/?]+/;
        if (data.reportname == '') {
            $('#id_error_crb_reportname')
                .html(M.util.get_string('emptyfullname', 'local_edwiserreports'))
                .show();
            ret = false;
        } else if (fullnameformate.test(data.reportname)) {
            $('#id_error_crb_reportname')
                .html(M.util.get_string('customreportfullnamewarning', 'local_edwiserreports'))
                .show();
            ret = false;
        }
    ...
    function customReportServiceInit() {
        ...
        // Search in table (global search across all columns, accessible input)
        // Maintainer note: This uses DataTables' global search API for client-side filtering.
        $('body').on('input', '.customreports-block-section .table-search-input input', function() {
            // If the input does not have a label, add one for accessibility
            if (!$(this).attr('aria-label')) {
                $(this).attr('aria-label', 'Search all columns');
            }
            // Use DataTables global search (searches all columns)
            if (cfPreviewTable) {
                cfPreviewTable.search(this.value).draw();
            }
        });
