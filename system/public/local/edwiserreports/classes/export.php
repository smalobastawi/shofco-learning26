...
class export {
    ...
    /**
     * Helper to clean and truncate filenames for export (max 200 chars, preserves extension)
    ...
    private function safe_filename($filename) {
        ...
        if (preg_match('/(\.[a-zA-Z0-9]+)$/', $filename, $m)) {
            $ext = $m[1];
            $basename = substr($filename, 0, -strlen($ext));
        } else {
            $basename = $filename;
        }
        if (strlen($filename) > 200) {
            $basename = substr($basename, 0, 200 - strlen($ext));
