...
class db_controller {
    ...
    public function fix_missing_course_progress() {
        ...
        if (empty($courses)) {
            mtrace('No courses found to process. Skipping course progress fix.');
            return;
        }

        $totalcourses = count($courses);
        mtrace("Found {$totalcourses} courses to process.");

        $progress = 0;
        $increament = 100 / $totalcourses;
    ...
    public function sync_old_users_with_course_progress() {
        ...
        // Check if there are any courses to process
        if (empty($courses)) {
            mtrace('No courses found to process. Skipping user sync.');
            return true; // Return true to indicate successful completion
        }

        mtrace("Found " . count($courses) . " courses to sync users for.");
