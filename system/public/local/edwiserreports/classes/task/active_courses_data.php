...
class active_courses_data extends \core\task\scheduled_task {
    ...
    public function execute() {
        try {
            ...
            // Check if there are any courses to process
            if (empty($courses)) {
                mtrace('No courses found to process. Skipping active courses data update.');
                set_config('activecoursesdata', json_encode([]), 'local_edwiserreports');
                return true;
            }
            ...
            mtrace("Fatal error in active_courses_data scheduled task: " . $e->getMessage());
        }
        // End progress.
