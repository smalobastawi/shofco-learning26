...
class active_users_data extends \core\task\scheduled_task {
    ...
    /**
     * @var string Filter for date range (e.g., 'last7days', 'weekly', etc.)
     */
    public $filter;

    /**
     * @var array Dates array for the current filter
     */
    public $dates;
    ...
    /**
     * @var int Number of x-labels (days) in the date range
     */
    public $xlabelcount;
        ...
        try {
            ...
            foreach ($filters as $filter) {
                try {
                    ...
                    mtrace("Error processing filter '{$filter}': " . $e->getMessage());
                }
            }
        } catch (\Throwable $e) {
            mtrace("Fatal error in active_users_data scheduled task: " . $e->getMessage());
        }
    }
