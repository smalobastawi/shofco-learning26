...
class update_course_progress_data extends \core\task\scheduled_task {
    ...
    /**
     * @var \local_edwiserreports\controller\progress Progress tracker for the task
     */
    public $progress;
     ...
     *
     * Handles errors gracefully and logs them using mtrace().
     */
    public function execute() {
        try {
            ...
            if (empty($progressdata)) {
                mtrace("No course progress records to update.");
                return;
            }
            ...
            foreach ($progressdata as $data) {
                try {
                    ...
                    // Log error for this record, but continue with others.
                    mtrace("Error updating course progress for record ID {$data->id}: " . $e->getMessage());
                }
            }
        } catch (\Throwable $e) {
            // Log fatal error for the whole task.
            mtrace("Fatal error in update_course_progress_data scheduled task: " . $e->getMessage());
        }
    }
}
