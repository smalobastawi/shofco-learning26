...
class completionblock extends block_base {
        ...
        if ($table) {
            ...
            foreach ($users as $key => $user) {
                ...
                $response[] = [
                    ...
                    'progress' => round(empty($user->progress) ? 0 : $user->progress, 2) . '%' . $link,
                    'grade' => round(empty($user->grade) ? 0 : $user->grade, 2) . '%',
            ...
            foreach ($users as $key => $user) {
                ...
                $data = [
                    ...
                    round(empty($user->progress) ? 0 : $user->progress, 2) . '%', // Progress.
                    round(empty($user->grade) ? 0 : $user->grade, 2) . '%', // Grade.
    ...
    public function get_summary_data($filters) {
        ...
        $timespent = $rtl ? date('s:i:H', mktime(0, 0, (int)$timespent)) : date('H:i:s', mktime(0, 0, (int)$timespent));
