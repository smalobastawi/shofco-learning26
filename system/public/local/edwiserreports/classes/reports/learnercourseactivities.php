    ...
    public function get_data($filters, $table = true) {
        ...
        foreach (array_keys($coursemodules) as $cmid) {
            ...
            if(!$table){
                ...
                $timespent = date($rtl ? 's:i:H' : 'H:i:s', mktime(0, 0, isset($gradetime->timespent) && is_numeric($gradetime->timespent) ? $gradetime->timespent : 0));
            }
            ...
            $data = [
                ...
                'grade' => round(empty($gradetime->finalgrade) ? 0 : $gradetime->finalgrade, 2),
                'gradedon' => $gradedon,
                'attempts' => $attempts,
                'highestgrade' => round(empty($highestgrade) ? 0 : $highestgrade, 2),
                'lowestgrade' => round(empty($lowestgrade) ? 0 : $lowestgrade, 2),
