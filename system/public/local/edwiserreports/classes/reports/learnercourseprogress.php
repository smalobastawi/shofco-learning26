...
class learnercourseprogress extends base {
        ...
        if ($table) {
            foreach ($courses as $id => $course) {
                ...
                $response[] = [
                    ...
                    "progress" => isset($course->progress) ? round($course->progress, 2) . '%' : '0%',
                    "grade" => isset($course->finalgrade) && is_numeric($course->finalgrade) ? round($course->finalgrade, 2) : 0,
            ...
            foreach ($courses as $id => $course) {
                ...
                $data = [
                    ...
                    isset($course->progress) ? round($course->progress, 2) . '%' : '0%', // Progress.
                    isset($course->finalgrade) && is_numeric($course->finalgrade) ? round($course->finalgrade, 2) : 0, // Grade.
    ...
    public function get_summary_data($filters) {
        ...
        if (empty($user->lastaccess)) {
            $lastaccess = get_string('never', 'local_edwiserreports');
        } else {
            if ($rtl) {
                // RTL format: "PM 19:09 2026 Jan 15"
                $ampm = date("A", $user->lastaccess);
                $hour = date("h", $user->lastaccess);
                $minute = date("i", $user->lastaccess);
                $year = date("Y", $user->lastaccess);
                $month = date("M", $user->lastaccess);
                $day = date("d", $user->lastaccess);
                $lastaccess = $ampm . ' ' . $minute . ':' . $hour . ' ' . $year . ' ' . $month . ' ' . $day;
            } else {
                // LTR format: "15 Jan 2026 09:19 PM"
                $lastaccess = date("d M Y h:i A", $user->lastaccess);
            }
        }
