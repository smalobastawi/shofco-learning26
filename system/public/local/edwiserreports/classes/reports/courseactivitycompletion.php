...
class courseactivitycompletion extends base {
        ...
        if ($table) {
            foreach ($users as $user) {
                $response[] = [
                    ...
                    'grade' => ($user->finalgrade === null ? '-' : (is_numeric($user->finalgrade) ? round($user->finalgrade, 2) : 'N/A')),
            ...
            foreach ($users as $user) {
                $data = [
                    ...
                    ($user->finalgrade === null ? '-' : (is_numeric($user->finalgrade) ? round($user->finalgrade, 2) : 'N/A')), // Grade.
                    ...
                    date($rtl ? 's:i:H' : 'H:i:s', mktime(0, 0, isset($user->timespent) && is_numeric($user->timespent) ? $user->timespent : 0)) // Timespent.
                ];
                // $data = [
                //     $user->fullname, // Learner.
                //     $user->email, // Email.
    ...
    /**
     * Get summary data for course activity completion.
     *
     * Calculates the total number of activities with completion enabled in the course,
     * the total number of completions by all enrolled users, and the overall completion rate.
     *
    ...
    /*public function get_summary_data($filters) {
        ...
        // Validate course.
        if (empty($filters->course) || !$DB->get_record('course', ['id' => $filters->course])) {
            throw new moodle_exception('invalidcourse', 'core_error');
        }
        ...
        $numactivities = count($cmids);
        
        // Get all enrolled users (using existing get_users method for consistency).
        $users = $this->get_users($filters);
        $userids = array_keys($users);
        $numusers = count($userids);

        $totalpossible = $numactivities * $numusers;
        $totalcompleted = 0;

        if ($numactivities > 0 && $numusers > 0) {
            // Count completions for all users and all activities.
            list($inusersql, $userparams) = $DB->get_in_or_equal($userids, SQL_PARAMS_NAMED, 'uid');
            list($incmsql, $cmparams) = $DB->get_in_or_equal($cmids, SQL_PARAMS_NAMED, 'cmid');
            $sql = "SELECT COUNT(1) AS completed
                      FROM {course_modules_completion}
                     WHERE userid $inusersql
                       AND coursemoduleid $incmsql
                       AND completionstate > 0";
            $params = array_merge($userparams, $cmparams);
            $totalcompleted = (int)$DB->get_field_sql($sql, $params);
        }

        $completionrate = ($totalpossible > 0) ? round(($totalcompleted / $totalpossible) * 100, 2) . '%' : '0%';

        // Compose summary output (add to or replace existing summary as needed)
        $summary = [
            [
                'title' => get_string('totalactivitieswithcompletion', 'local_edwiserreports'),
                'data' => $numactivities
            ],
            [
                'title' => get_string('totalusers', 'local_edwiserreports'),
                'data' => $numusers
            ],
            [
                'title' => get_string('totalcompletions', 'local_edwiserreports'),
                'data' => $totalcompleted
            ],
            [
                'title' => get_string('overallcompletionrate', 'local_edwiserreports'),
                'data' => $completionrate
            ]
        ];
        ...
        // Optionally merge with other summary data as needed (visits, grades, etc.)
    ...
    public function get_summary_data($filters) {
        ...
        $avgtimespent = $rtl ? date('s:i:H', mktime(0, 0, (int)$avgtimespent)) : date('H:i:s', mktime(0, 0, (int)$avgtimespent));
