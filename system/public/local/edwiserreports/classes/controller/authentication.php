...
class authentication {
    ...
    public function delete_secret_key(?int $userid = null, ?string $secret = null) {
        ...
        if ($secret !== null) {
            return $DB->delete_records('edwreports_authentication', ['secret' => $secret]);
        }
        ...
        return $DB->delete_records('edwreports_authentication', ['userid' => $userid]);
    }
    ...
    public function get_secret_key(?int $userid = null): string {
        ...
        if ($record = $DB->get_record('edwreports_authentication', ['userid' => $userid])) {
    ...
    public function get_user(string $secret): int {
        ...
        if ($record = $DB->get_record_sql($sql, [$secret])) {
            return $record->userid;
        }

        // Return 0 if secret key does not exist (Moodle standard: return 0 instead of false for int return type).
        return 0;
    }
}
