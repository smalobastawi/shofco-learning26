...
class block_base {
    ...
    /**
     * Get old date range for comparison (previous period).
     * @param String $timeperiod Timeperiod
     * @param int $startdate Current period start date
     * @param int $enddate Current period end date
     */
    public function get_old_date_range($timeperiod, $startdate, $enddate) {
        // Old end date is one day before current start date.
        ...
        switch ($timeperiod) {
            ...
            case 'yearly':
                ...
                // If current period is Apr 2024 - Mar 2025, old period is Apr 2023 - Mar 2024.
     ...
     *
     * Date Range Logic:
     * - last7days: Yesterday to 7 days ago (NOT including today)
     * - weekly: Last week Sunday to Saturday
     * - monthly: First day to last day of previous month
     * - yearly: Financial year April 1 to March 31
     */
    public function get_date_range($timeperiod) {

        // Default enddate - end of yesterday (start of today in UTC terms).
        ...
        switch ($timeperiod) {
            case 'last7days':
                ...
                // If today is Feb 3, shows Jan 27 to Feb 2 (7 days).
                $enddate = floor(strtotime('yesterday') / 86400 + 1) * 86400;
                $days = LOCAL_SITEREPORT_WEEKLY_DAYS - 1;
                break;
            case 'weekly':
                // Weekly days. From Last Week. Sunday to Saturday.
                // Get last Saturday as end date.
                $enddate = floor(strtotime('last saturday') / 86400 + 1) * 86400;
                // Get Sunday of that week as start date (6 days before Saturday).
                $days = LOCAL_SITEREPORT_WEEKLY_DAYS - 1;
                break;
            case 'monthly':
                // Monthly days. Last Month's 1st day to last day.
                // If today is Feb 1, shows Jan 1 to Jan 31.
                $enddate = strtotime('last day of previous month 23:59:59');
                $days = floor($enddate / 86400) - floor(strtotime('first day of previous month') / 86400);
                break;
            case 'yearly':
                // Financial Year: April 1 to March 31.
                // If today is before April, use previous financial year.
                // Ex. Today is Feb 1, 2026: Shows Apr 1, 2024 to Mar 31, 2025.
                // Ex. Today is May 1, 2026: Shows Apr 1, 2025 to Mar 31, 2026.
                $month = date('m');
                $year = date('Y');
                if ($month < 4) {
                    // Before April, use previous financial year.
                    $year--;
                }
                $enddate = strtotime("$year-03-31 23:59:59") + 1;
                $days = floor($enddate / 86400) - floor(strtotime(($year - 1) . "-04-01") / 86400) - 1;
                break;
            default:
                ...
                // If it has correct startdate and end date then count xlabel.
        ...
        // Calculating startdate based on enddate and days.
    ...
    public function get_user_from_cohort_course_group($cohortid, $courseid, $groupid, $userid = null) {
        ...
        // Guard against orphaned course references (e.g. deleted courses still present in
        // edwiserreports tables or filters). Without this check, get_course() throws a
        // MUST_EXIST exception and breaks scheduled tasks such as studentengagement_data.
