...
function local_edwiserreports_create_manage_icons_for_emaillist($id, $blockname, $region, $enable) {
    $manage = html_writer::start_span("row esr-manage-scheduled-emails m-0 p-0 justify-content-center d-flex");
    ...
    $manage .= html_writer::link('javascript:void(0)',
        ...
        array(
            "class" => "esr-email-sched-edit col-4",
    ...
    $manage .= html_writer::link('javascript:void(0)',
        ...
        array(
            "class" => "esr-email-sched-delete col-4",
            "data-blockname" => $blockname,
            "data-region" => $region,
            "data-id" => $id,
            "data-sesskey" => sesskey(),
...
function local_edwiserreports_create_toggle_switch_for_emails($id, $emailenable, $blockname, $region, $customclass = '') {
    ...
    $switchparams = array(
        ...
        "href" => "#",
        "data-id" => $id,
        "class" => "mr-3 d-flex col-4 esr-email-toggle esr-switch",
        "data-sesskey" => sesskey(),
        "data-blockname" => $blockname,
        "data-region" => $region,
...
function local_edwiserreports_get_required_strings_for_js() {
    ...
    local_edwiserreports_require_beacon();
}

/**
 * Ensure HelpScout Beacon script is loaded for report surfaces.
 * Only visible to site administrators.
 */
function local_edwiserreports_require_beacon() {
    global $PAGE, $USER;

    // Only show beacon to site administrators.
    if (!is_siteadmin()) {
        return;
    }
    ...
    static $beaconqueued = false;

    if ($beaconqueued || empty(LOCAL_EDWISERREPORTS_BEACON_ID)) {
        return;
    }

    $beaconid = LOCAL_EDWISERREPORTS_BEACON_ID;

    // Inject HelpScout Beacon script directly using inline JavaScript in global scope.
    $beaconscript = "(function(e,t,n){
        function a(){
    ...
    $PAGE->requires->js_init_code($beaconscript, false);

    // Add CSS to adjust HelpScout Beacon icon position (move left and up) and hide square box background.
    $beaconcssscript = "
    (function(){
        var style = document.createElement('style');
        style.type = 'text/css';
        style.innerHTML = '/* HelpScout Beacon position adjustment */ ' +
            '#beacon-container { ' +
                'right: 80px !important; ' +
                'bottom: 100px !important; ' +
                'z-index: 999999 !important; ' +
            '} ' +
            '/* Hide square box background but keep container visible for functionality */ ' +
            '.BeaconContainer, ' +
            '.BeaconContainer.is-configDisplayRight, ' +
            '.ContainerFramecss__WrapperUI-sc-d7rzhh-0 { ' +
                ...
                'margin: 0 !important; ' +
            '} ' +
            '/* Show button frame with proper positioning */ ' +
            ...
            '} ' +
            '/* Hide gradient container background */ ' +
            '.ContainerGradient-sc-62zoy-0 { ' +
                'background: transparent !important; ' +
    ...
    $PAGE->requires->js_init_code($beaconcssscript, false);
    $beaconqueued = true;
}
