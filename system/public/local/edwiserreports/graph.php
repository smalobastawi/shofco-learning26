...
// Add polyfill inline in head to ensure it loads before any scripts
...
foreach (scandir($CFG->dirroot . '/local/edwiserreports/externaljs/build/') as $file) {
    if (pathinfo($file, PATHINFO_EXTENSION) != 'js' || $file == 'jspdf-polyfill.js') {
        continue;
    }
    $jsfiles[] = $file;
}
// Sort to ensure jspdf-polyfill loads first, then other files
sort($jsfiles);
